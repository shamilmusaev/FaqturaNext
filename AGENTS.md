...
Tech stack: Next.js 15 (App Router) + React 19 + TypeScript (strict) + Tailwind v4 + Supabase (Postgres + Auth + Storage) + next-intl (EN + SV). UI primitives are hand-built --- no shadcn. Radix used only for headless a11y (Dialog, etc.). Money is `bigint` cents (Ã¶re for SEK).
...
### Money (P0)
- Money is stored and passed around as `bigint` cents (Ã¶re for SEK). Never `number` for sums, never `numeric`/`float` for running totals.
...
- VAT calculations round per line item to whole Ã¶re (Skatteverket requirement). Flag any cross-line rounding.
...
- Swedish status terms (`Faktura skickad`, `Betald`, `FÃ¶rfallen`, `Utkast`, `Avbruten`) stay in Swedish even on EN UI --- by brand decision.
...
### Accessibility (P1)
- Touch targets -‰¥ 44Ã—44px (`min-h-11 min-w-11`).
