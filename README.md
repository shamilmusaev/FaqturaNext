...
- **Bilingual where natural**. Status chips and key product terms can appear in Swedish when it's clearer (*Faktura skickad*, *Betald*, *Förfallen*, *ROT/RUT*) — but the UI chrome is English-first so the system reads as a *product*, not a translation.
...
**Type.**
- **Hanken Grotesk** — one family for both UI and display. A serious, neutral grotesque in the Söhne / GT America family. Differentiated by *weight* and *tracking*, not by family change:
  - Display (hero numbers, section heroes): 600 · `-0.03em`
  - UI / body: 400–500 · `-0.015em` at large sizes, normal at body size
  - Buttons / labels: 500
...
Faqtura uses **Lucide** at 1.5px stroke (CDN). Icons inherit `currentColor`. Sizes: 16 (dense UI), 20 (nav, buttons), 24 (empty states, large CTAs). The brand mark is a stylized "ƒ" (lowercase f with a forward bar) — see `assets/logo.svg`. No emoji. No unicode glyphs as icons.
