-- Drop old signature first; create_or_replace cannot change parameter list.
drop function if exists create_invoice(uuid, uuid, date, text, text, jsonb);
...
begin
  ...
  insert into invoices (organization_id, client_id, number, status, issued_at, due_at, currency, notes)
  values (p_org, p_client_id, v_number, 'draft', v_issued_at, p_due_at, p_currency, p_notes)
  ...
  loop
    ...
    v_amount_cents := round((v_item->>'quantity')::numeric * (v_item->>'unit_price_cents')::bigint);
    ...
    values (
      ...
      (v_item->>'unit_price_cents')::bigint, v_vat_rate, v_amount_cents
    );
...
revoke all on function create_invoice(uuid, uuid, date, text, text, jsonb, date) from public;
grant execute on function create_invoice(uuid, uuid, date, text, text, jsonb, date) to authenticated;
