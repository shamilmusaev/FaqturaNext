-- Optional per-invoice flag to hide the OCR reference from the preview and PDF.
-- The OCR is still generated and stored; hide_ocr only controls rendering.

alter table invoices add column hide_ocr boolean not null default false;
...
begin
  ...
  values (
    ...
    faqtura_ocr(v_number), coalesce(p_hide_ocr, false), coalesce(p_reverse_vat, false), v_rot_rut_type, coalesce(p_rot_rut_cents, 0),
