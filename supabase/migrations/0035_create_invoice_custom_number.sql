-- Allow an optional manual invoice number (p_number). When blank, fall back to
-- the auto sequence. Uniqueness is still enforced by the (organization_id,
-- number) constraint, so a duplicate manual number surfaces as an error.
