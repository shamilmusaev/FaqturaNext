...
-- Also snapshot the version on invoice creation (first autosave).
...
begin
  ...
  -- Snapshot the initial version so the user can always go back to square one.
  perform save_invoice_version(v_invoice.id);
